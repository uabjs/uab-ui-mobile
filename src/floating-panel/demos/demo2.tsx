...
import { FloatingPanel, Avatar, Space, Card, List, FloatingPanelRef } from 'uab-ui-mobile'
import {
...
export default () => {
  ...
  return (
    <>
      <iframe src='https://amap.com' className={'floating-panel-map'} />

      <FloatingPanel anchors={anchors} ref={ref}>
        <Space block className={'floating-panel-search'}>
          {/* <SearchBar
          ...
          /> */}
          {!focus && <Avatar src='' className={'floating-panel-avatar'} />}
        </Space>
        <List header='Favorites' style={{ '--border-bottom': 'none' }}>
          <Card>
            <Space block justify='around'>
              {locations.map(item => (
                <div key={item.name} className={'floating-panel-item'}>
