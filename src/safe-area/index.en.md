...
| ---------- | ------------------ | ------- | --------------------------- |
| --multiple | Displayed multiple | `1`     | `--uabm-safe-area-multiple` |

## FAQ

### In the rem layout, what should I do if the height of the SafeArea is small?

Some rem schemes will set the overall scaling of the web page through the viewport meta. In this case, the SafeArea will also be scaled correspondingly, so the height is likely to be abnormal. At this time, you can adjust the display multiplier of SafeArea through the global `--uabm-safe-area-multiple` CSS variable. For example, if your project framework scales the entire page to 0.5 times, then you can set `--uabm -safe-area-multiple: 2` to make corresponding compensation adjustments.
